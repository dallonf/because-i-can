...
use bevy::{math::const_vec2, sprite::collide_aabb::Collision};
use gdnative::prelude::godot_print;

use crate::{
    bevy_app::GodotTime,
    gd_sync::{GD2DTransform, GDSprite, GDSpriteBundle},
};

// These constants are defined in `Transform` units.
// Using the default 2D camera they correspond 1:1 with screen pixels.
// The `const_vec3!` macros are needed as functions that operate on floats cannot be constant in Rust.
const PADDLE_SIZE: Vec2 = const_vec2!([120.0, 20.0]);
const GAP_BETWEEN_PADDLE_AND_FLOOR: f32 = 60.0;
const PADDLE_SPEED: f32 = 500.0;
// How close can the paddle get to the wall
const PADDLE_PADDING: f32 = 10.0;

const BALL_STARTING_POSITION: Vec2 = const_vec2!([0.0, -50.0]);
...
fn setup(mut commands: Commands) {
    ...
    commands
        .spawn()
        .insert(Paddle)
        .insert_bundle(GDSpriteBundle {
            transform: GD2DTransform {
                position: Vec2::new(0.0, paddle_y),
                scale: PADDLE_SIZE,
                ..default()
            },
            sprite: GDSprite {
    ...
    commands
        ...
        .insert_bundle(GDSpriteBundle {
            ...
            sprite: GDSprite {
                color: BALL_COLOR,
                z_index: 1, // We set the z-value of the ball to 1 so it renders on top in the case of overlapping sprites.
                ..default()
            },
            ..default()
        })
...
fn apply_velocity(mut query: Query<(&mut GD2DTransform, &Velocity)>, time: Res<GodotTime>) {
...
fn check_for_collisions(
    ...
    mut ball_query: Query<(&mut Velocity, &GD2DTransform), With<Ball>>,
    collider_query: Query<(Entity, &GD2DTransform, Option<&Brick>), With<Collider>>,
    ...
    for (collider_entity, transform, maybe_brick) in collider_query.iter() {
        let collision = collide(
            ball_transform.position.extend(0.0),
            ball_size,
