...
# set up
players = [{'number': i, 'hand': []} for i in range(NUM_PLAYERS)]
deck = list(CARDS)
random.shuffle(deck)
...
def take_turn(player, card):
    ...
    if card['rank'] in groups.keys():
        ...
        if smallest_group['count'] < size_of_match:
            old_card = next(c for c in player['hand'] if c['rank'] == smallest_group['rank'])
